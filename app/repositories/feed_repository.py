...
def _visibility_condition(current_user_id: UUID):
    ...
    return or_(
        Follow.status == "ACCEPTED",#contas privadas: só traz se o status do follow for "ACCEPTED", contas públicas: o status do follow "ACCEPTED" by default, então a condição acima já cobre.
    )
...
def get_following_feed_query(current_user_id: UUID): # Recebe o id do usuário logado, que é quem está pedindo o feed "Seguindo"
    ...
    return (
        ...
        .where(
            ...
            AppUser.deleted_at.is_(None), #não aparece recordas de usuários deletados no feed
        )
