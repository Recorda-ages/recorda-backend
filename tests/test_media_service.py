...
def test_upload_media_persists_and_returns_response(db):
    response = media_service.upload_media(db, JPEG_BYTES)
...
def test_upload_media_rejects_unsupported_type(db):
    with pytest.raises(media_service.UnsupportedMediaTypeError):
        media_service.upload_media(db, b"not a real file")


def test_upload_media_rejects_oversized_file(db):
    oversized = b"\xff\xd8\xff" + b"\x00" * MAX_UPLOAD_SIZE_BYTES
    with pytest.raises(media_service.MediaTooLargeError):
        media_service.upload_media(db, oversized)


def test_upload_media_wraps_storage_failures(db):
    with patch.object(
        media_storage_repository, "save", side_effect=RuntimeError("boom")
    ):
        with pytest.raises(media_service.MediaStorageError):
            media_service.upload_media(db, JPEG_BYTES)
